import { getDevelopmentDatabase } from ".";
...
export const insertChannel = async (channel: Channel) => {
  const db = getDevelopmentDatabase();
  return db.query(
    "INSERT INTO channels (channel_id, channel_name) VALUES ($1, $2) RETURNING *",
...
export const upsertChannel = async (channel: Channel) => {
  const db = getDevelopmentDatabase();
  return db.query(
    "INSERT INTO channels (channel_id, channel_name) VALUES ($1, $2) ON CONFLICT (channel_id) DO UPDATE SET channel_name = $2 RETURNING *",
    [channel.channelId, channel.channelName]
  );
};
...
export const updateChannel = async (updatedChannel: Channel) => {
  const db = getDevelopmentDatabase();
  return db.query(
    "UPDATE channels SET channel_name=$2 WHERE channel_id=$1 RETURNING *",
    [updatedChannel.channelId, updatedChannel.channelName]
  );
};
